...
test("collapsed reasoning does not mount the reasoning text", () => {
  assert.match(source, /\{expanded \? block\.thinking : null\}/);
  assert.match(source, /useState\(false\)/);
  assert.doesNotMatch(source, />\s*\{block\.thinking\}\s*</);
});
