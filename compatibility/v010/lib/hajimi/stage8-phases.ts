...
export async function moveStage8(cwd:string,next:Stage8Phase,note:string,figurePaths?:string[],severeIssues:string[]=[],repairPaths:string[]=[]){
 return withTaskLock(cwd,async()=>{
  ...
  if(b<0)throw new Error('Invalid stage8 phase');
  if(b<a){
   if(next==='figures'){
    s.repairPaths=repairPaths;
   }
   // Reopen first: missing prerequisites must not gate the repair itself.
   // Forward transitions still check all untouched artifacts and outputs.
   if(next==='figures'||next==='writing')delete s.paperHashes;
  }else if(b===a){if(next==='figures'&&repairPaths.length)s.repairPaths=repairPaths;else return s;}
