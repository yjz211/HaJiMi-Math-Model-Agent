...
!define HAJIMI_INSTALLER_LOG_DIR "$TEMP\HaJiMi"
!define HAJIMI_INSTALLER_LOG_FILE "${HAJIMI_INSTALLER_LOG_DIR}\installer.log"

!macro appendInstallerLog MESSAGE
  Push $R9
  CreateDirectory "${HAJIMI_INSTALLER_LOG_DIR}"
  FileOpen $R9 "${HAJIMI_INSTALLER_LOG_FILE}" a
...
!macro logInstallDirProcesses
  ${If} $INSTDIR != ""
    nsExec::Exec '"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -ExecutionPolicy Bypass -Command "Add-Content -LiteralPath \"${HAJIMI_INSTALLER_LOG_FILE}\" -Value \"Processes running from install dir: $INSTDIR\"; Get-Process | Where-Object { $$_.Path -and $$_.Path.StartsWith(\"$INSTDIR\", [System.StringComparison]::OrdinalIgnoreCase) } | Select-Object Id,Name,Path | Out-File -LiteralPath \"${HAJIMI_INSTALLER_LOG_FILE}\" -Append -Width 4096"'
    Pop $R0
...
!macro closeAppProcesses
  ...
  !insertmacro closeAppProcess "HaJiMi.exe" "HaJiMi"
...
!macro closeInstallDirProcesses
  ${If} $INSTDIR != ""
    ...
    nsExec::Exec '"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -ExecutionPolicy Bypass -Command "$$logFile = \"${HAJIMI_INSTALLER_LOG_FILE}\"; $$dir = \"$INSTDIR\"; $$targets = Get-Process | Where-Object { ($$_.Path -and $$_.Path.StartsWith($$dir, [System.StringComparison]::OrdinalIgnoreCase)) -or ($$_.Name -eq \"HaJiMi\" -and $$_.CommandLine -and $$_.CommandLine.IndexOf($$dir, [System.StringComparison]::OrdinalIgnoreCase) -ge 0) }; Add-Content -LiteralPath $$logFile -Value \"Killing $$($$targets.Count) install-dir process(es)\"; $$targets | Select-Object Id,Name,Path | Out-File -LiteralPath $$logFile -Append -Width 4096; $$targets | ForEach-Object { Stop-Process -Id $$_.Id -Force -ErrorAction SilentlyContinue }; Start-Sleep -Seconds 1; $$remain = Get-Process | Where-Object { ($$_.Path -and $$_.Path.StartsWith($$dir, [System.StringComparison]::OrdinalIgnoreCase)) -or ($$_.Name -eq \"HaJiMi\" -and $$_.CommandLine -and $$_.CommandLine.IndexOf($$dir, [System.StringComparison]::OrdinalIgnoreCase) -ge 0) }; Add-Content -LiteralPath $$logFile -Value \"After kill: $$($$remain.Count) process(es) still alive\""'
    Pop $R0
...
!macro customInstall
  nsExec::ExecToStack '"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "$INSTDIR\resources\hajimi\runtime\windows\provision-runtime.ps1"'
  Pop $R0
  Pop $R1
  ${If} $R0 != 0
    MessageBox MB_ICONSTOP "HaJiMi runtime directory provisioning failed. $R1"
    Abort
  ${EndIf}
