...
import { resolveHajimiRuntimePaths } from "./hajimi-runtime";

// ---------------------------------------------------------------------------
// Single Instance Lock
// ---------------------------------------------------------------------------
// Test profiles must be selected before Chromium storage and the instance lock.
const configuredUserData = process.env.HAJIMI_USER_DATA?.trim();
if (configuredUserData) {
  if (!path.isAbsolute(configuredUserData)) throw new Error("HAJIMI_USER_DATA must be absolute");
  mkdirSync(configuredUserData, { recursive: true });
  app.setPath("userData", configuredUserData);
  app.setPath("sessionData", configuredUserData);
  app.setAppLogsPath(path.join(configuredUserData, "logs"));
}
...
function startNextServer(port: number): ServerProcess {
  ...
  const {
    ...
    productRoot: hajimiProductRoot,
    runtimeHome: hajimiRuntimeHome,
  } = resolveHajimiRuntimePaths({
  ...
  if (isDev) {
    ...
    const proc = spawn("node", [nextBin, "dev", "-p", String(port)], {
      ...
      env: {
        ...
        PI_CODING_AGENT_DIR: hajimiAgentDir,
        HAJIMI_PRODUCT_ROOT: hajimiProductRoot,
        HAJIMI_RUNTIME_HOME: hajimiRuntimeHome,
        HAJIMI_PROJECTS_ROOT: process.env.HAJIMI_PROJECTS_ROOT,
        HAJIMI_MANAGED_DESKTOP: "1",
      },
      stdio: "pipe",
    });
  ...
  const proc = process.platform === "darwin"
    ? (() => {
        const utility = utilityProcess.fork(serverScript, [], {
          ...
          env: serverEnv,
          stdio: "pipe",
          serviceName: "HaJiMi Next Server",
        });
  ...
  try {
    await loadPageWithRetry(
      ...
      async (url) => {
        ...
        {
          // Cold development compilation can exceed the packaged-page timeout.
          attemptTimeoutMs: app.isPackaged ? 15_000 : 120_000,
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    if (app.isPackaged && existsSync(updateConfigPath)) {
      ...
      logInfo("Automatic update check disabled: no HaJiMi update configuration");
    }
  } catch (err) {
    serverState = "stopped";
    cleanup();
