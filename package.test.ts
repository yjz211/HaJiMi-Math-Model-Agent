...
test("HaJiMi identity, Pi runtime versions, and product resources are pinned", () => {
  ...
  for (const resource of ["bundled", "toolkit", "runtime"]) {
    assert.ok(pkg.files.includes(resource), `${resource} must be included in the npm package`);
  }
});

test("build scripts name the standalone Next.js build explicitly", () => {
  assert.match(pkg.scripts["build:standalone"], /^npm run check:capabilities && next build\b/);
  assert.match(
...
test("standalone Pi runtime closes over the dynamically exported server package", () => {
  const script = readFileSync(
    new URL("./scripts/ensure-standalone-pi-runtime.mjs", import.meta.url),
    "utf8",
  );
  assert.match(script, /roots\s*=\s*\[[\s\S]*?"@earendil-works\/pi-server"/);
});

test("packaging and release scripts call build:standalone", () => {
  assert.match(pkg.scripts.release, /npm run build:standalone/);
  assert.match(pkg.scripts.pack, /^npm run check:windows-runtime && npm run build:standalone &&/);
  assert.match(pkg.scripts.dist, /^npm run check:windows-runtime && npm run build:standalone &&/);
...
test("next.config tracing excludes tests and separately packaged runtime archives", () => {
...
test("packaged smoke test targets Windows and architecture-suffixed macOS outputs", () => {
  ...
  const standaloneSmoke = readFileSync(
    new URL("./scripts/smoke-standalone-server.mjs", import.meta.url),
    "utf8",
  );
  assert.match(standaloneSmoke, /PI_CODING_AGENT_DIR: isolatedAgentDir/);
  assert.match(standaloneSmoke, /HAJIMI_AGENT_DIR: isolatedAgentDir/);
  assert.match(standaloneSmoke, /HAJIMI_PRODUCT_ROOT: sourceProductRoot/);
  assert.match(standaloneSmoke, /const authorizedSessionDir = join\(isolatedAgentDir, "sessions"/);
  assert.match(standaloneSmoke, /type: "session", version: 3/);
  assert.match(standaloneSmoke, /\/api\/hajimi\/status\?cwd=/);
  assert.match(standaloneSmoke, /afterStatus !== beforeStatus/);
  assert.match(script, /check-hajimi-resource-parity\.mjs/);
  assert.match(script, /packagedOutput\.productRoot/);
});
