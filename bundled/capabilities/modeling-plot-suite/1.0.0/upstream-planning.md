...
按问题分 reasoning/data/mixed，写每图一句核心信息、具体图型、配方编号、理由、源文件和章节。DATA 图优先选择最接近的现有编号配方作为构图骨架；可以按本题数据修改图层、布局和标注，只有现有配方明显不适合表达核心关系时才使用 custom。数据量撑不起独立画布时，合并为同一论证链的子图、改用更紧凑的标准图或取消该图。以上只约束 DATA，不限制 DRAWIO/TIKZ 的自定义构图。先选完整图集，再批量预取配方，逐图适配。
...
- 对每个子问题主动识别可视化的几何构造、变量映射、事件递推、约束关系、算法状态转换和求解链；只要技术图有助于读者理解模型如何建立、公式如何连接或算法如何推进，就优先使用 TIKZ，不能因为正文也能描述而自动省略。每个推理密集问题必须有忠实求解过程的 TIKZ 推导构造图，混合题按真实推导需要积极安排。选定后必须呈现本题特有的对象、关系和步骤，不能退化为一排公式框、几个孤立区间或通用流程模板；动手前先确定视觉中心、主辅关系和阅读路径，优先采用与推导语义一致的空间构造、分层递推、状态循环或主图加局部解释。内容较少时使用紧凑二维构图或与同一推导链的相关结构合成完整图，不得靠拉长横向画布制造空白。不能用泛泛示意代替推导，也不能为数据图数量省掉验证、关键结果或灵敏度表达；缺少相关真实结果则在规划解释，不能捏造。
- 推理密集问题的数据图达到硬底线即可，不为18–30硬加重复曲线。图注/正文承载长说明，图内只留必要短标注。
- 默认沿用原版按数据形态选图。expressive 用户模式按 `profiles/expressive.md` 的适用门槛增加明亮层次和有实际信息收益的表达变化，不在此重复加重；其他模式不增加图型创新要求。
