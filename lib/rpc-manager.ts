...
export class AgentSessionWrapper {
  ...
  applyAgentMode(mode: AgentMode): void {
    ...
    this._agentMode = mode;
    this._modeRef.current = mode;
    const tools = withHajimiTools(
      withMemoryTools(effectiveToolsForMode(mode, this._toolPreset), mode),
      mode,
    );
  ...
  async send(command: Record<string, unknown>): Promise<unknown> {
    ...
    switch (type) {
      ...
      case "set_model": {
        ...
        return { id: model.id, provider: model.provider, thinkingLevel: this.inner.agent.state?.thinkingLevel ?? "off" };
      }
      ...
      case "set_thinking_level": {
        const requested = command.level as string;
        const level = requested === "auto" ? (this.inner.settingsManager.getDefaultThinkingLevel() ?? "medium") : requested;
        ...
        return { thinkingLevel: this.inner.agent.state?.thinkingLevel ?? level };
      }
...
export async function startRpcSession(
  ...
  const starting = (async () => {
    ...
    const effectiveTools = withHajimiTools(
      withMemoryTools(effectiveToolsForMode(agentMode, toolPreset), agentMode),
      agentMode,
    );

    const modeRef: AgentModeRef = { current: agentMode };
    const productRoot = process.env.HAJIMI_PRODUCT_ROOT ?? process.cwd();
    const resourceLoader = new DefaultResourceLoader(hajimiResourceLoaderOptions({
      cwd,
      agentDir,
      productRoot,
      extensionFactories: [
        hajimiCoreInlineExtension({ cwd, productRoot, getMode: () => modeRef.current }),
