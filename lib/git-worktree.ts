...
import { runManagedGit } from "./hajimi/managed-runtime.mjs";
...
function runGitProcess(args: string[], options: { cwd: string }): Promise<GitCommandResult> {
  if (process.platform === "win32" && process.env.HAJIMI_MANAGED_DESKTOP === "1") return runManagedGit(options.cwd, args);
