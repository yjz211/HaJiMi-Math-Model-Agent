...
test("macOS packaging emits universal DMG and ZIP artifacts", () => {
  ...
  assert.match(config, /artifactName: "HaJiMi-\$\{version\}-mac-\$\{arch\}\.\$\{ext\}"/);
  assert.doesNotMatch(
    config,
    /^electronDist:/m,
...
test("Linux packaging emits a deb artifact", () => {
  ...
  assert.match(config, /executableName: hajimi/);
  assert.match(config, /artifactName: "HaJiMi-\$\{version\}-linux-\$\{arch\}\.\$\{ext\}"/);
});
...
test("HaJiMi modeling resources are packaged outside the asar archive", () => {
  for (const [from, to] of [
    ["bundled", "hajimi/bundled"],
    ["toolkit", "hajimi/toolkit"],
    ["runtime", "hajimi/runtime"],
  ]) {
    assert.match(config, new RegExp(`- from: ${from}\\n\\s+to: ${to}`));
  }
});

test("packaged HaJiMi toolkit excludes local Python test caches", () => {
  const start = config.indexOf("- from: toolkit\n");
  const end = config.indexOf("- from: runtime\n", start);
  assert.ok(start !== -1 && end > start, "toolkit extraResources block expected");
  const block = config.slice(start, end);
  for (const pattern of ["!**/__pycache__", "!**/*.py[cod]", "!**/.pytest_cache"]) {
    assert.ok(block.includes(`- \"${pattern}\"`), `toolkit filter must contain ${pattern}`);
  }
});
