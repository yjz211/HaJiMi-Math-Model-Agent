...
export async function executionEnvironment(runtime, { cacheRoot, cwd, toolkitRoot, capabilitiesRoot, productRoot }) {
  ...
  for (const p of [home, texvar, join(cacheRoot, 'matplotlib'), join(cacheRoot, 'fontconfig')]) await mkdir(p, { recursive: true });
  return {
    ...
    PYTHONPATH: [join(runtime.root, 'support'), toolkitRoot,
