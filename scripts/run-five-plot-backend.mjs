...
const cwd = join(productRoot, 'projects/five-synthetic-20260909');
const trace = join(productRoot, 'artifacts/five-synthetic-20260909');
...
if (!resume) {
  ...
  await copyFile(join(productRoot, 'scripts/generate-five-plot-datasets.py'), join(cwd, 'src/generate_data.py'));
...
本次是独立绘图测试，宿主只加载绘图技能和真实 HaJiMi Windows 工作区工具，不加载整篇论文状态机。无需研究阶段验收、证据冻结、八图或路线图；技能的 full-paper 条款不适用本请求。保留选图规划、配方引用、图像检查与必要修正，写 PAPER_PLAN.md/FIGURE_MANIFEST 和 BACKEND_RESULT.md，记录错误及修正。不要改产品源码或启动论文写作。
技能原文路径：${skill}/SKILL.md。宿主没有提供配方答案或绘图代码。`;

const actualPrompt = resume ? '继续本次五图测试，沿用已有输入和产物，完成看图修正及结果记录。' : prompt;
