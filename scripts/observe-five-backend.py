...
trace=root/'artifacts/five-synthetic-20260909'
...
print(json.dumps({'events':len(rows),'assistantTurns':len(messages),'tools':dict(Counter(r['tool'] for r in starts)),
 'errors':len(errors),'latestEvent':rows[-1]['time'],'outputs':len(list((root/'projects/five-synthetic-20260909/figures').glob('*.pdf')))},ensure_ascii=False))
for row in rows[-8:]:
