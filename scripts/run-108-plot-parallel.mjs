...
function update(){refresh=refresh.then(async()=>{await gallery();await writeFile(join(out,'parallel-status.json'),JSON.stringify({pid:process.pid,concurrency:18,updatedAt:new Date().toISOString(),batches:states},null,2));}).catch(e=>console.error(e.message));return refresh;}
await writeFile(join(out,'parallel-controller.pid'),String(process.pid));
const timer=setInterval(()=>void update(),30000);
async function run(b){
 const tag=String(b).padStart(2,'0'),cwd=join(root,`projects/plot108-20260910/batch-${tag}`),trace=join(out,`batch-${tag}`);
 await mkdir(trace,{recursive:true});
 const prior=await readJSON(join(trace,'run-result.json'),{});
 if(prior.reason==='completed'){states[tag]={status:'completed'};return;}
 if(adopted.has(tag)){
  const pid=Number(adopted.get(tag));states[tag]={status:'running',pid,adopted:true};await update();
  while(true){const r=await readJSON(join(trace,'run-result.json'),{});if(r.finishedAt){states[tag]={status:r.reason,pid,adopted:true};return;}
   try{process.kill(pid,0);}catch{states[tag]={status:'needs-attention',pid,reason:'adopted process exited without result'};return;}
   await pause(5000);
  }
 }
 ...
 states[tag]={status:'running',pid:child.pid,resume};console.log('START',tag,child.pid);
 let log='';for(const stream of [child.stdout,child.stderr])stream.on('data',data=>{log+=data.toString();});
 const code=await new Promise(resolve=>{child.on('error',e=>{log+=e.stack;resolve(-1)});child.on('close',resolve)});
 await writeFile(join(trace,'parallel-child.log'),log);
 const result=await readJSON(join(trace,'run-result.json'),{});states[tag]={status:code===0?(result.reason??'exited'):'needs-attention',pid:child.pid,exitCode:code};await update();console.log('END',tag,states[tag].status);
}
try{await Promise.allSettled(Array.from({length:18},(_,i)=>run(i+1)));}finally{clearInterval(timer);await update();await writeFile(join(out,'parallel-controller.pid'),'0');}
